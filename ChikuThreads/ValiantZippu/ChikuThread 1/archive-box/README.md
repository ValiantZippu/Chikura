...
This is the **expanding workspace** for Chikura (知蔵) — each ChikuThread's inbox. Dump raw URLs here, then collaboratively sort them into the clean domain files in `vault/` (e.g. `vault/music.md`). Valid ChikuThread requires `archive-box/inbox.md`.

## Why it exists

- Vault files (`vault/music.md`, `vault/games.md`, `vault/gadget-electronics.md`, etc) are **curated, clean, deduped** — one resource, one place, with `## Section` / `### Category` / `- https://` structure.
- New finds, bulk imports, unsorted dumps, or questionable links **do not go straight to vault**. They go here first. This preserves curation and makes progress traceable.
