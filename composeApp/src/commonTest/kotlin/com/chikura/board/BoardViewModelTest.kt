...
class BoardViewModelTest {
    ...
    fun moveResourceSectionChanged() {
        ...
        val chikuthread = buildChikuThread("sample", "sample", mapOf("music" to markdown))
        // Verify initial id
        assertEquals("Sound", chikuthread.domains.first().resources.first().section)
        assertEquals("music-0001", chikuthread.domains.first().resources.first().id)

        val vm = BoardViewModel(chikuthread, mapOf("music" to markdown))
        vm.move("music-0001", "Music Theory")
        val after = vm.getChikuThread()
    ...
    fun preserveIndent() {
        val markdown = "## Sound\n### EQ\n    - https://youtu.be/RIuqjFP2cHg?si=xxx\n    - https://a.com\n\n## DAW\n\n- https://b.com\n"
        val chikuthread = buildChikuThread("sample", "sample", mapOf("music" to markdown))
        val first = chikuthread.domains.first().resources.find { it.url.contains("RIuqjFP2cHg") }!!
        ...
        // Section changed
        val byUrl = vm.getChikuThread().domains.flatMap { it.resources }.find { it.url.contains("RIuqjFP2cHg") }!!
        assertEquals("Music Theory", byUrl.section)
    }
    ...
    fun moveToNewSectionCreatesHeader() {
        val markdown = "# Music\n\n## Sound\n\n- https://a.com\n"
        val chikuthread = buildChikuThread("test", "test", mapOf("music" to markdown))
        val vm = BoardViewModel(chikuthread, mapOf("music" to markdown))
        vm.move("music-0001", "Music Theory")
        val md = vm.getMarkdown("music")
        assertTrue(md.contains("## Music Theory"))
        val byUrl = vm.getChikuThread().domains.flatMap { it.resources }.find { it.url.contains("a.com") }!!
        assertEquals("Music Theory", byUrl.section)
    }
}
