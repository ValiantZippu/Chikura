 ...
 *
 * - data class ChikuThreadMeta etc (see model/ChikuThread.kt — @Serializable with marketplace fields)
 * - fetchMarketplace(): List<ChikuThreadMeta> from marketplace.json (curated index), Json decode
...
// Curated index — not an open store. Hosted as raw json in the Chikura org.
// Desktop fallback can load local marketplace.json from classpath/resources if network fails.
const val MARKETPLACE_URL = "https://raw.githubusercontent.com/ValiantZippu/Chikura/main/marketplace.json"
const val MARKETPLACE_FALLBACK_URL = "https://raw.githubusercontent.com/Chikura/marketplace/main/marketplace.json"
...
/**
 * Returns true if [fileName] is kebab-case *.md per chikuthread spec (e.g. music.md, gadget-electronics.md).
 ...
 * Used by AddChikuThreadDialog to gate clone.
 */
...
/** Convenience: true if URL is a plausible GitHub chikuthread URL. */
fun isValidGithubChikuThreadUrl(url: String): Boolean = parseGithubUrl(url) != null

// --- chikuthread structure validation ---

/**
 * Validates that a set of repo file paths (relative to repo root) satisfies Chikura chikuthread spec:
 * - At least one kebab-case *.md at root (e.g. music.md) OR in allowed subdirs, but spec says root *.md
 * - Must contain archive-box/inbox.md (the inbox) — quarantine.md is optional but should exist for valid curated chikuthreads
 *
 ...
 */
data class ChikuThreadValidationResult(val isValid: Boolean, val reason: String = "")

fun validateChikuThreadStructure(paths: List<String>): ChikuThreadValidationResult {
    ...
    if (!hasInbox) {
        return ChikuThreadValidationResult(false, "Missing required archive-box/inbox.md")
    }
    ...
    if (curatedRootMd.isEmpty()) {
        return ChikuThreadValidationResult(false, "No kebab-case *.md at repo root (e.g. music.md)")
    }
    val invalidKebab = curatedRootMd.filter { !isKebabCaseMd(it) }
    if (invalidKebab.isNotEmpty()) {
        return ChikuThreadValidationResult(false, "Non kebab-case markdown at root: ${invalidKebab.joinToString(", ")} (must be kebab-case)")
    }
    ...
    return ChikuThreadValidationResult(true, "Valid chikuthread (${curatedRootMd.size} domains, inbox present)")
}

/**
 * Shorthand for validating a single file list string from git tree / GitHub API.
 */
fun validateChikuThreadFileListFromApi(fileNames: List<String>): ChikuThreadValidationResult =
    validateChikuThreadStructure(fileNames)

// --- marketplace JSON fetch ---

/**
 * Decode marketplace json string (curated index) to List<ChikuThreadMeta>.
 * Ignores unknown keys, lenient.
 *
 * Example marketplace.json:
 * [
 *   {"id":"music-chikuthread","name":"Music ChikuThread","url":"https://github.com/Chikura/music-chikuthread","description":"..."},
 *   {"id":"games-chikuthread","name":"Games ChikuThread","url":"https://github.com/Chikura/games-chikuthread"}
 * ]
 *
 * Test spec:
 * @Test fun parsesMarketplace() { val list = Json.decodeFromString<Marketplace>(json); assertEquals(1, list.size) }
 * where Marketplace == List<ChikuThreadMeta> (typealias in model/ChikuThread.kt)
 */
fun decodeMarketplace(json: String): List<ChikuThreadMeta> {
    if (json.isBlank()) return emptyList()
    return try {
        marketplaceJson.decodeFromString<Marketplace>(json)
    } catch (_: Exception) {
        // Also try wrapper object { "chikuthreads": [...] } for forward compat
        try {
            val wrapper = marketplaceJson.decodeFromString<MarketplaceWrapper>(json)
            wrapper.chikuthreads
        } catch (_: Exception) {
            emptyList()
        }
    }
...
private data class MarketplaceWrapper(val chikuthreads: List<ChikuThreadMeta> = emptyList())
 ...
 *
 * This is the primary "curated, not open store" index. Direct add-by-URL is alternative for private chikuthreads.
 */
suspend fun fetchMarketplace(client: HttpClient): List<ChikuThreadMeta> {
 ...
 */
suspend fun fetchMarketplace(): List<ChikuThreadMeta> {
...
/**
 * Build raw.githubusercontent.com URL for a chikuthread file.
