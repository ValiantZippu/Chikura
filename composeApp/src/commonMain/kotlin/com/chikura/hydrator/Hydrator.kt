 ...
 * cache to chikuthreads/<id>/.chikura-cache/media.json (gitignored) + in-memory via simple file cache.
 */
 ...
 * Default cache is per-"default" chikuthread sidecar — callers with known chikuthreadId can
 * create their own MediaCache(chikuthreadId) and call hydrateWithCache().
 */
object Hydrator {
    // In-memory + file sidecar cache. Lazy-load on first use.
    private var _cache: MediaCache = MediaCache("default", FileSystem.homeChikuThreadsPath()).also {
