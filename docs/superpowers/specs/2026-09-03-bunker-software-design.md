# Chikura Software + Web Wiki — Design

> Status: Draft for review. No implementation until approved. This spec is the single source for the cross-platform “blank app + loadable megathread chikuthreads” vision.

## 1. Summary

Chikura is a **blank-by-default, cross-platform app** (Windows, macOS, Linux, Android) plus a **read-only web wiki** with the same black & white terminal look. It loads any GitHub repo that follows the Chikura chikuthread spec (kebab-case Markdown + archive-box). Each chikuthread is a curated megathread — a moelist-type catalog but modern, monospace, high-contrast, not colorful. The app solves the current pain: Markdown is a perfect database (git, forever) but unusable for 2000 YouTube cards (no previews, no drag), while Discord gave previews but was tedious to keep updated and unmigratable.
...
- Any chikuthread that follows the spec loads; curated marketplace stays tiny, not huge.

**Non-Goals (v1):**
- No user accounts, no in-chikuthread comments (use GitHub issues), no hosting videos, no search beyond Fuse.js, no open store.

## 3. Architecture — Where ChikuThreads Live

```
Installed app (blank on first launch)
 └─ ~/Chikura/
     ├─ chikuthreads/
     │   ├─ chikura/      # git clone ValiantZippu/Chikura (default)
     │   └─ japan-chikuthread/         # git clone user/repo (added via URL)
...
- Each chikuthread = **separate GitHub repo**, not subfolder inside Chikura. Clone is the “download template” action.
- App blank by default; user picks local root on launch. “Add ChikuThread” pre-fills `ValiantZippu/Chikura`.
...
```json
[{ "id": "japan-chikuthread", "repo": "user/japan-chikuthread", "name": "Japan ChikuThread", "author": "user", "description": "..." }]
```
...
## 5. Valid ChikuThread Spec (what makers follow)

```
my-chikuthread/
  LICENSE
  README.md
  chikuthread.json?  { name, version, description, author }
...
> Note
```
Validator: at least one `*.md` with `#`, `archive-box/inbox.md` exists, bullets parseable. Missing `chikuthread.json` → “Untitled ChikuThread”.
...
- **App (Tauri, editable):** Split pane: Left Monaco editor on real `.md` (Ctrl+], vim, diff) + Right Canvas (List / Kanban columns=`##` / Whiteboard nodes+edges via tldraw/dnd-kit). Bidirectional: drag card right → moves bullet left. Video expand: LiteYouTube thumb → iframe + title/channel/description/comments, cached to `.chikura-cache`. Git commit button.
...
- **Hydrate:** For each URL, fetch `https://noembed.com/embed?url=` (or YouTube Data API) → `{title, thumbnail_url, author_name}`. Save to `.chikura-cache/media.json` (gitignored) + in-memory SQLite. Web fetches live. `.md` stays `- https://youtu.be/xxxx`.
...
- Manual QA: load chikura, add second chikuthread via URL, drag inbox card, expand video, pull update.
