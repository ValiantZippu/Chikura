...
A thread is an **author-scoped folder** of markdown. In this repo every thread lives under `ChikuThreads/<github-author>/<thread-name>/`; an author can equally host a thread as its own GitHub repo with the same inner layout — the loader treats both the same.

```
ChikuThreads/
└── <author>/                  ← e.g. ValiantZippu — discovery is keyed to authors
    └── <thread-name>/         ← e.g. ChikuThread 1 — one thread folder per thread
...
- **Author-owned, never merged.** Content is organized by author; no thread is copied into another thread or into a marketplace — the curated index (`marketplace.json`) only points at the thread's home.
- `archive-box/inbox.md` and `archive-box/quarantine.md` **must exist** (can be empty) — they power the intake workflow. Other archive files (processed-log, dumps) are allowed alongside them.
- Domain files are **kebab-case `*.md`** at the *thread* root, one per topic. No deep folder spaghetti.
...
```json
{
  ...
  "author": "author",
  "description": "…",
  "url": "https://github.com/author/japan-thread",
  "githubUrl": "https://github.com/author/japan-thread"
}
```
